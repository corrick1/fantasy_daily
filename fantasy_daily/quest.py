...
def daily_claim(token, address, auth_data, account_number):
    try:
        daily_logger = logging.getLogger('daily_claim')
        daily_logger.setLevel(logging.INFO)
        fh = logging.FileHandler('daily_claim.log')
        ...
        payload = {
            "questId": "db5c6df1-1106-4853-93d5-c6ec60d1a03d" #YOUR_ID#######################
        }

        response = requests.post('https://www.fantasy.top/api/quests/complete', json=payload, headers=headers)

        if response.status_code == 200:
            data = response.json()
            daily_logger.info(f'Successful claim for account {account_number}: {address}')
            daily_logger.info(json.dumps(data, indent=2))
        else:
            daily_logger.error(f'Daily claim failed for account {account_number}: {address}!')
            daily_logger.error(response.text)
    except requests.exceptions.RequestException as e:
        daily_logger.error(f'Error during daily claim request for account {account_number}: {address}', exc_info=True)
